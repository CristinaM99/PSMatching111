...
It also calculates the measures of performance in the unmatched cohort and in the pairwise matched cohorts;
(ii) the macro function 'import_match111' imports the datasets containing the triplets of patients 
(one treated subjects and two controls, one per control group) matched (in R), and calculates the measures of performance 
in the three-way matched cohort;
(iii) three macro functions (SD_unmatched_controls, SD_match11_PS_ctrl, SD_match111_controls) that allow 
to calculate the overall standardized distance (OSD) between the two gorups of controls 
...
%macro simulation(n_simul,n_pz,prev_A,prev_B,prev_C);
	...
	%put iteration &i;
		...
		*data generating process;
		data simul.simul&i;
			*outcome probability in center A (treated subjects);
			rand_pA = rand("Bernoulli",0.5); 
			if rand_pA = 0 then pA = 0.1;
			else if rand_pA = 1 then pA = 0.25;
			*risk ratio of treated subjects (RRtreated);
			alpha = pA/(1-pA); 
		...
		*unmatched cohort;
		**control B vs treated subjects contrast;
		DATA tab_unmatchedB;
		simul = &i; 
		RUN;
		***sample size: n = &n_pz;
		...
		***relative bias(%), reduction in bias(%) and mean square error (MSE);
		...
		run;
		**control C vs treated subjects contrast;
		DATA tab_unmatchedC;
		simul = &i; 
		RUN;
		***sample size: n = &n_pz;
		...
		**extended common support (CS) and caliper computation;
		...
		run;
		**selection of the subjects (treated subjects and controls) which relies inside the CS;
		data pz_centerA_cs;
		set pz_centerA;
		...
		run;
		**treated subjects ordered by decreasing pA (probability of being treated in center A with the experimental treatment);
...
%macro SD_match11_PS_ctrl(n_simul);
	...
	%put iteration &i;
		...
		run;
		proc delete data=common_treated;
		run;
